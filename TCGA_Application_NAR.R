...
if(FULL_DATA){
  patients <- sample(patientsX, length(patientsX))
}else{
...
res2       <- HC_fused_subtyping_ens(list(mRNA), 
              max.k=5, 
...
# Perform the genetic algorithm
res1       <- HC_fused_subtyping_ga2(list(mRNA))
...
if(do.TAPIO){
 ...
 # Clinical enrichment
 if(CLIN){
 CLIN_TAPIO[[xx]] <-  check.clinical.enrichment(cl_TAPIO, 
                        subtype.name=cancertype)
 }
}
...
if(do.TAPIO){
groups <- factor(cl_TAPIO)
names(groups) = rownames(survival)
coxFit <- coxph(Surv(time = Survival, event = Death) ~ groups, data = survival, ties="exact")
cox_nemo <- round(summary(coxFit)$sctest[3],digits = 40);
#print(cox_fused)
P_TAPIO[xx] = round(summary(coxFit)$sctest[3],digits = 40);
K_TAPIO[xx] = length(unique(groups))
C_TAPIO[xx] = summary(coxFit)$concordance 
}

RESX <- cbind(P_SNF,P_PINS,P_NEMO,P_FUSED, P_PAREA1, P_PAREA2, P_TAPIO)
#RESX <- cbind(K_SNF,K_PINS,K_NEMO,K_FUSED,K_PAREA1,K_PAREA2)
#RESX <- cbind(C_SNF,C_PINS,C_NEMO,C_FUSED,C_PAREA1,C_PAREA2, C_TAPIO)
colnames(RESX) <- c("SNF","PINS","NEMO","HCFUSED","PAREA1","PAREA2","TAPIO")
print(RESX)
